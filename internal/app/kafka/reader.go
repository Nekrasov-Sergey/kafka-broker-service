...
import (
	...
	"kafka-broker-service/internal/app/telegram"
	"log"
	"os"
	"strconv"
	"time"
...
func (k *Reader) FetchMessage(ctx context.Context) error {
	for {
		...
		case <-time.After(1 * time.Second):
			...
			_ = k.Reader.SetOffset(int64(ioff))
			...
			if n == 3 {
				_ = k.Reader.SetOffset(k.Reader.Offset() - 1)
